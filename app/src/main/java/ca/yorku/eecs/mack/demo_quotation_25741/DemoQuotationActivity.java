...
public class DemoQuotationActivity extends Activity implements View.OnTouchListener
{
    ...
    //Variables for the searchbar/search functionality
    SearchView search;
    String searchQuery;
    ListView listView;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        //Setting the search bar/search functionality
        listView = findViewById(R.id.listView);
        adapter = new ArrayAdapter<>(this, android.R.layout.simple_list_item_1,names);
        listView.setAdapter(adapter);
        listView.setVisibility(View.GONE);

        //Old code, leaving it here if I need to implement this into the search or for debug reasons
        /*listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                String selected = parent.getItemAtPosition(listView.getFirstVisiblePosition() + position).toString();
                List hold = Arrays.asList(names);
                myBackStack.add(quotationIdx); // idx of quote that *was* showing
                quotationIdx = hold.indexOf(selected); // new Idx
                showingBack = false;
        ...
        //Sets up the listeners for the searchView and the code for the methods
        search.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            @Override
            //When the user clicks on the search button in the soft keyboard
            public boolean onQueryTextSubmit(String query) {
                ...
                List hold = Arrays.asList(names);
                //Checks if the name in the query is in the app's database (quotations.xml)
                try {
                    //re using code from showNameListDialog()
                    myBackStack.add(quotationIdx); // idx of quote that *was* showing

                    /*
                    if index is not found (meaning that the name in the query doesn't exist),
                    it will throw an IndexOutOfBoundsException
                    */
                    quotationIdx = hold.indexOf(listView.getItemAtPosition(listView.getFirstVisiblePosition())); // new Idx
                    showingBack = false;
                    ...
                    //When the user types a name that doesn't exist in the app's database (quotations.xml)
                    Toast.makeText(getApplicationContext(), "Not Found", Toast.LENGTH_SHORT).show();
                    return false;
                }
            }
            //When the user is typing something into the searchbar/searchView
            @Override
            public boolean onQueryTextChange(String newText) {
                Boolean check = search.getQuery().length()==0;
                //if there is nothing in the query, the listView should not be displayed
                if (check){
                    listView.setVisibility(View.GONE);
                } else if(listView.getVisibility()!=View.VISIBLE){
                    listView.setVisibility(View.VISIBLE);
                }
                //gets and filters the names based what is in the query
