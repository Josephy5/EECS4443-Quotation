...
public class QuizActivity extends Activity implements View.OnTouchListener, ResultsDialog.OnResultsDialogClickListener
{
	...
	/*
	 * this variable is created for the testing methods in the DemoQuotationActivityTest
	 * It holds locations of the correct answers for all the questions in the quiz
	 */
	static int[] junitTestArrCorrectAnswer;
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		if (savedInstanceState == null) // the activity is being created for the 1st time
		{
			...
			//initializes array variable
			junitTestArrCorrectAnswer= new int[numberOfQuestions];
			for (int i = 0; i < q.length; ++i) {
				q[i] = new Question(temp[i], quotation.length, NUMBER_OF_ANSWERS);
				//adds the correct answer to the created array variable
				junitTestArrCorrectAnswer[i] = q[i].getCorrectAnswer();
			}
	...
	/*
	 * this get method is created for the testing methods in the DemoQuotationActivityTest
	 *
	 * Provides DemoQuotationActivityTest the answers for the quizXXXX testing methods
	 */
	public static int[] getJunitTestArrCorrectAnswer(){
		return junitTestArrCorrectAnswer;
	}
